...
void TileMap::RenderLayer(int layer, int cameraX, int cameraY){
	...
	for(y=0; y < mapHeight; y++){
		for(x=0; x < mapWidth; x++){
			if(At(x, y, layer)!=-1){
				int destinyX= ParallaxScrolling((int)x*tileSet->GetTileWidth(),cameraX, layer);
				int destinyY= ParallaxScrolling((int)y*tileSet->GetTileHeight(), cameraY, layer);
				//int destinyX = (int)x*tileSet->GetTileWidth() + cameraX;
				//int destinyY = (int)y*tileSet->GetTileHeight() + cameraY;
				tileSet->RenderTile(At(x, y, layer), destinyX, destinyY);
				//tileSet->RenderTile(At(x, y, layer), x*tileSet->GetTileWidth(), y*tileSet->GetTileHeight());
			}
		}
	}
...
int TileMap::ParallaxScrolling(int pos, int camera, int layer){
	return (int)(pos-camera*(layer+1));
}